...
if (accountSid && accountSid.startsWith('AC') && authToken && !authToken.includes('placeholder') && !authToken.includes('here')) {
...
const { authenticateToken: authMiddleware, authenticateOptional, restrictTo } = require('./middleware/auth');
...
REQUIRED_ENV.forEach(key => {
  if (!process.env[key]) {
    console.error(`[FATAL] Missing required environment variable: ${key}. Add it to your .env file.`);
    process.exit(1);
  }
});
    ...
    try {
      ...
      if (existing) {
        ...
        await dbRun(
          ...
          [destinationId, `Destination #${destinationId}`, 'destination', 'Global', videoBuffer, videoMime, videoUrl]
        );
      }
      ...
      res.status(500).json({ success: false, error: 'Failed to save video to database: ' + dbErr.message });
    }
  });
});
...
app.post('/api/destinations/:id/image', (req, res, next) => {
  imageUpload.single('image')(req, res, async (err) => {
    ...
    try {
      ...
      if (existing) {
        await dbRun(
          'UPDATE destinations SET image = ? WHERE id = ?',
      ...
      res.status(500).json({ success: false, error: 'Failed to save image to database: ' + dbErr.message });
    }
  });
});
...
app.post('/api/auth/signup-request', validate(schemas.signupRequest), asyncHandler(async (req, res) => {
  ...
  await dbRun(
    ...
    [email, passwordHash, sanitize(fullName), sanitize(username), phoneNumber || null, code, expires]
  );
  ...
  if (process.env.SMTP_PASS) {
    try {
      ...
      throw createError('Failed to send email verification code. Please check your email address.', 500);
    }
  } else {
...
app.post('/api/auth/signup-verify', validate(schemas.signupVerify), asyncHandler(async (req, res) => {
  ...
  const existingUsername = await dbGet('SELECT id FROM users WHERE username = ?', [pending.username]);
  if (existingUsername) throw createError('This username is already taken.', 409);
  ...
  const result = await dbRun(
    ...
    [pending.email, pending.password_hash, pending.full_name, pending.username, 'USD', defaultAvatar, pending.phone_number]
  );
...
app.post('/api/auth/register', validate(schemas.register), asyncHandler(async (req, res) => {
  const { email, password, fullName, username, phoneNumber } = req.body;
  ...
  const result = await dbRun(
    'INSERT INTO users (email, password_hash, full_name, username, currency, avatar_url, phone_number) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [email, passwordHash, sanitize(fullName), sanitize(username), 'USD', defaultAvatar, sanitize(phoneNumber || '')]
  );
  ...
  res.status(201).json({
    ...
    message: 'Registration successful',
    token,
    user: { id: userId, email, fullName: sanitize(fullName), username: sanitize(username), currency: 'USD', avatarUrl: defaultAvatar, phoneNumber: sanitize(phoneNumber || '') },
  });
}));
...
app.post('/api/auth/login', validate(schemas.login), asyncHandler(async (req, res) => {
  ...
  const user = await dbGet('SELECT * FROM users WHERE email = ? OR username = ?', [email.toLowerCase(), email]);
...
app.put('/api/user/profile', authenticateToken, async (req, res, next) => {
  ...
  try {
    ...
    if (otherUser) {
      return res.status(409).json({ error: 'Email or username is already taken by another account.' });
    }

    await dbRun(
      'UPDATE users SET full_name = ?, username = ?, email = ?, currency = ?, phone_number = ? WHERE id = ?',
      [sanitize(fullName), sanitize(username), email.toLowerCase(), safeCurrency, sanitize(phoneNumber || ''), req.user.id]
    );
...
function getCorrectImagePath(dbImage) {
  ...
  if (dbImage.startsWith('http://') || dbImage.startsWith('https://') || dbImage.startsWith('//') || dbImage.startsWith('assets/')) {
    return dbImage;
  }
...
app.get('/api/destinations/:id/video', async (req, res, next) => {
  try {
    ...
    const row = await dbGet('SELECT video_url, video_blob, video_mime FROM destinations WHERE id = ?', [id]);
...
app.post('/api/create-payment-intent', authenticateToken, validate(schemas.createPaymentIntent), createPaymentIntent);
...
app.patch('/api/bookings/:id/guest-info', authenticateToken, validate(schemas.guestInfo), updateGuestInfo);
...
app.put('/api/bookings/:id/auto-confirm', async (req, res) => {
  try {
    ...
    if (booking.status !== 'AWAITING_CONFIRMATION') {
      return res.json({ success: true, message: 'Booking already processed.', status: booking.status });
    }
    await dbRun(
      `UPDATE bookings SET status = 'SUCCESS' WHERE id = ?`,
      [bookingId]
    );
...
app.put('/api/admin/bookings/:id/verify', authenticateToken, restrictTo('admin'), verifyBookingPayment);

app.get('/api/admin/users', authenticateToken, restrictTo('admin'), async (req, res, next) => {
  try {
    const users = await dbAll('SELECT id, name, email, role, avatar, created_at FROM users ORDER BY created_at DESC');
...
app.put('/api/admin/users/:id/role', authenticateToken, restrictTo('admin'), async (req, res, next) => {
  try {
    const { id } = req.params;
    const { role } = req.body;
...
app.delete('/api/admin/users/:id', authenticateToken, restrictTo('admin'), async (req, res, next) => {
  try {
    const { id } = req.params;
...
app.get('/api/admin/stats', authenticateToken, restrictTo('admin'), async (req, res, next) => {
  try {
    ...
    const totalBookings = await dbGet('SELECT COUNT(*) as count, COALESCE(SUM(total_price), 0) as total_revenue FROM bookings');
    const pendingBookings = await dbGet('SELECT COUNT(*) as count FROM bookings WHERE status IN ("PENDING", "AWAITING_CONFIRMATION")');
    res.json({
      success: true,
      stats: {
...
app.post(
  ...
  async (req, res) => {
    ...
    if (webhookSecret && sig) {
      ...
      console.warn('[Webhook] ⚠️  STRIPE_WEBHOOK_SECRET not set — skipping signature verification (dev mode only).');
      try {
    ...
    if (!hostelListingId || !bookingId) {
      ...
      console.log('[Webhook] PaymentIntent has no hostelListingId/bookingId metadata — not a hostel booking. Skipping payout.');
...
app.post('/api/feedback', authenticateToken, async (req, res, next) => {
  ...
  try {
    ...
    fs.promises.writeFile(feedbackFilePath, JSON.stringify(feedbackArray, null, 2), 'utf8')
      .catch(e => console.error('[Feedback File Write Error]', e));

    res.status(201).json({ message: 'Feedback submitted successfully. Thank you!', feedbackId: result.lastID });
  } catch (err) {
    next(err);
  }
});
