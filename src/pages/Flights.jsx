...
import { Plane, Calendar, Users, MapPin, Compass, Search, ArrowRight, ShieldCheck } from 'lucide-react';
    ...
    >
      ...
      <div className="max-w-6xl mx-auto space-y-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2">
          <div>
            <div className="text-[10px] text-slate-500 font-mono tracking-widest uppercase mb-2">Aerospace Grid</div>
            <h1 className="text-5xl md:text-6xl font-black text-white tracking-tighter leading-none">
              Supersonic <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-blue-500">Transits</span>
            </h1>
            <p className="text-slate-400 font-medium tracking-wide mt-2">Book private orbital shuttles and hyper-sonic flight paths.</p>
          </div>
          <NeonBadge variant="blue" className="w-fit">Supersonic Fleet Online</NeonBadge>
        </div>
        ...
        >
          <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="flex flex-col space-y-2">
              <label className="text-[9px] text-slate-400 font-mono font-bold tracking-widest uppercase">Origin</label>
            ...
            {/* Destination input */}
            <div className="flex flex-col space-y-2">
              <label className="text-[9px] text-slate-400 font-mono font-bold tracking-widest uppercase">Destination</label>
              <div className="relative flex items-center">
            ...
            {/* Date input */}
            <div className="flex flex-col space-y-2">
              <label className="text-[9px] text-slate-400 font-mono font-bold tracking-widest uppercase">Departure Date</label>
              <div className="relative flex items-center">
              ...
              <TicketStubSummary 
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                      <div className="text-[10px] text-slate-500 font-mono tracking-widest uppercase mb-1 flex items-center gap-1">
                        <ShieldCheck className="w-3.5 h-3.5 text-blue-400" /> SECURE CONCIERGE TRANSLATION
                      </div>
                      <h3 className="text-3xl font-black text-white tracking-tighter">
                        Boarding Pass Locked
                      </h3>
                      <p className="text-xs text-slate-400 mt-1">Route: {searchState.origin} <ArrowRight className="inline w-3 h-3 text-slate-500" /> {searchState.destination}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-2 border-t md:border-t-0 md:border-l border-slate-800/60 pt-4 md:pt-0 md:pl-8 text-left">
                      <div>
                        <span className="block text-[9px] text-slate-500 font-mono tracking-widest uppercase">Operator</span>
                        <span className="text-sm font-extrabold text-white">{selectedFlight.operatorName}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] text-slate-500 font-mono tracking-widest uppercase">Vector ID</span>
                        <span className="text-sm font-extrabold text-blue-400 font-mono">{selectedFlight.routeIdentifier}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] text-slate-500 font-mono tracking-widest uppercase">Cabin Class</span>
                        <span className="text-sm font-extrabold text-amber-500">{selectedFlight.chosenClass}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] text-slate-500 font-mono tracking-widest uppercase">Travelers</span>
                ...
                rightContent={
                  <div className="text-center">
                    <div className="text-[10px] text-slate-500 font-mono tracking-widest uppercase mb-1">Elite Charter Price</div>
                    <div className="text-4xl font-black text-white tracking-tighter mb-5">${selectedFlight.fare}</div>
                    <ShimmerButton className="w-full !py-3.5 !text-xs !rounded-xl">Confirm Flight Vector</ShimmerButton>
                  </div>
                }
              />
            </motion.div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-6">
            {mockFlights.map(flight => {
              ...
              const transitWithClass = isSelected ? { ...flight, chosenClass: selectedFlight.chosenClass } : flight;
