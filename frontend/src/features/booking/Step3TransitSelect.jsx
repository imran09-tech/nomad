...
import { Train, Bus, Loader2, AlertCircle, CheckCircle2, Search, MapPin, Utensils, ShieldCheck, Star } from 'lucide-react';
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <div className="inline-flex items-center space-x-2 bg-gray-900 border border-gray-700 px-3 py-1.5 rounded-full text-xs text-gray-300">
            ...
            <span>Monitoring Live Station: <strong>{activeStationCode}</strong></span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
          {filteredOptions.map((transit) => {
            ...
            if (transit.type === 'Train') {
              const depTime = new Date(transit.departureTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
              const arrTime = new Date(transit.arrivalTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
              
              const basePrice = transit.pricingTier || 450;
              const trainClasses = [
                { name: '3A', price: Math.round(basePrice * 2.6), status: 'WL 81', prob: '34% Chance', isGreen: false },
                { name: '2A', price: Math.round(basePrice * 3.7), status: 'WL 23', prob: 'Waitlist', isGreen: true },
                { name: '1A', price: Math.round(basePrice * 6.2), status: 'WL 10', prob: 'Waitlist', isGreen: true },
                { name: 'SL', price: basePrice, status: 'WL 74', prob: '40% Chance', isGreen: false }
              ];

              return (
                <div key={transit.id} className="rounded-2xl bg-[#0d111a] border border-slate-800/80 p-5 hover:border-slate-700/80 transition-all flex flex-col relative mb-2">
                  {isSelected && (
                  ...
                  <div className="flex justify-between items-center my-4 pb-4 border-b border-slate-900">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-slate-400 font-semibold">{originCity.substring(0,4).toUpperCase()}</span>
                    </div>
                    <div className="flex flex-col items-center flex-1 px-8">
                      <span className="text-xs text-slate-400 font-medium mb-1">— {transit.durationString} →</span>
                      <div className="w-full h-[1px] bg-slate-800"></div>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="text-lg text-white font-bold">{arrTime}</span>
                      <span className="text-sm text-slate-400 font-semibold">{destinationCity.substring(0,4).toUpperCase()}</span>
                    </div>
                  ...
                  <div className="flex space-x-3 overflow-x-auto pt-2 pb-1 custom-scrollbar">
                    {transit.classes ? Object.entries(transit.classes).map(([className, price], idx) => {
                      const isClassSelected = isSelected && bookingContext.transitData?.chosenClass === className;
                      
                      return (
                        <div 
                          key={idx}
                          onClick={(e) => { e.stopPropagation(); handleSelect(transit, className, price); }}
                        ...
                        >
                          <div className="text-xs text-slate-400 font-bold mb-1">{className}</div>
                          <div className="text-sm font-extrabold text-white">₹{price}</div>
                        </div>
                      );
                    }) : trainClasses.map((cls, idx) => {
                      const isClassSelected = isSelected && bookingContext.transitData?.chosenClass === cls.name;
                      
                      return (
                        <div 
                          key={idx}
                          onClick={(e) => { e.stopPropagation(); handleSelect(transit, cls.name, cls.price); }}
                          className={`min-w-[110px] border rounded-xl p-3 text-left transition-all cursor-pointer relative ${
                            isClassSelected 
                              ? 'border-amber-500 bg-[#1c2436] ring-1 ring-amber-500/20' 
                              : 'bg-[#121824] border-slate-800 hover:border-slate-700'
                        ...
                        >
                          <div className="text-xs text-slate-400 font-bold mb-1">{cls.name}</div>
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-6">
                  <div className={`p-3 rounded-full ${isSelected ? 'bg-amber-500 text-black' : 'bg-gray-800 text-gray-400'}`}>
                    <Bus className="w-6 h-6" />
                  </div>
                  
                  <div>
                    <h4 className={`font-bold ${isSelected ? 'text-white' : 'text-gray-200'}`}>
                      {transit.operatorName} <span className="text-xs font-normal text-gray-500 ml-2">#{transit.routeIdentifier}</span>
                    </h4>
                    <div className="flex items-center space-x-4 mt-2 text-sm text-gray-400">
                      <div className="flex flex-col">
                        <span className="text-white font-medium">{new Date(transit.departureTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                      ...
                      <div className="flex flex-col">
                        <span className="text-white font-medium">{new Date(transit.arrivalTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                      ...
                      {transit.platform && (
                        <div className="flex flex-col pl-4 border-l border-gray-800">
                          <span className="text-amber-500 font-medium text-xs border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 rounded">
                            {transit.delay !== '0 Mins' ? `Delay: ${transit.delay}` : 'Live • On Time'}
                          </span>
