...
export default function KineticTransitCard({
  transit,
  originCity,
  destinationCity,
  type = 'flight',
  ...
  const depTime = new Date(transit.departureTime || new Date()).toLocaleTimeString([], {
    hour: '2-digit',
    minute: '2-digit',
  });
  const arrTime = new Date(transit.arrivalTime || new Date()).toLocaleTimeString([], {
    hour: '2-digit',
    minute: '2-digit',
  });
      ...
      <div className="p-6">
        ...
        <motion.div layout className="flex justify-between items-center mb-8">
          <div className="flex items-center space-x-3">
            <div
              className={`w-8 h-8 rounded-full bg-${baseColor}-500/10 flex items-center justify-center border border-${baseColor}-500/20`}
            >
              <Icon className={`w-4 h-4 text-${baseColor}-500`} />
            </div>
            <div className="font-extrabold text-white text-lg tracking-wide">
              {transit.operatorName || 'SkyLux Airlines'}{' '}
              <span className="text-slate-500 font-mono text-sm ml-2">
                {transit.routeIdentifier || 'FL-492'}
              </span>
            </div>
          </div>
        ...
        <motion.div layout className="flex justify-between items-center">
          <div className="flex flex-col items-start w-24">
            ...
            <span className="text-xs text-slate-500 font-bold tracking-widest uppercase mt-1">
              {originCode}
            </span>
          </div>
              ...
              {Object.entries(transit.classes).map(([className, price], idx) => {
                ...
                return (
                  <motion.div
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      onSelect(transit, className, price);
                    }}
                  ...
                  >
                    ...
                    <div className="text-[10px] text-slate-400 font-mono font-bold tracking-widest mb-2 uppercase">
                      {className}
                    </div>
                    <div className="text-2xl font-black text-white tracking-tighter">${price}</div>
                    <div className="mt-4 flex justify-between items-center">
                      <NeonBadge variant="emerald" className="!px-1.5 !py-0.5 !text-[8px]">
                        Available
                      </NeonBadge>
                    </div>
                  </motion.div>
