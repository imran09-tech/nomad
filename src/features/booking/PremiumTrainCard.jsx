...
export default function PremiumTrainCard({ train, originCity, destinationCity, isSelected, onSelect }) {
  const [isExpanded, setIsExpanded] = useState(false);

  // Extract times
  const depTime = new Date(train.departureTime || new Date()).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
  const arrTime = new Date(train.arrivalTime || new Date()).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
      ...
      <div className="p-6">
        ...
        <motion.div layout className="flex justify-between items-center mb-6">
          <div className="font-bold text-white text-lg tracking-tight">
            {train.routeIdentifier || train.id} <span className="text-slate-400 font-medium ml-2">{train.operatorName || 'Express'}</span>
          </div>
        ...
        <motion.div layout className="flex justify-between items-center">
          ...
          <div className="flex flex-col items-center flex-1 px-8 relative">
            <span className="text-xs text-slate-500 font-medium mb-2 tracking-widest uppercase">{train.durationString || '4h 30m'}</span>
              ...
              {Object.entries(train.classes).map(([className, price], idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs text-slate-400 font-bold tracking-wider mb-1 uppercase">{className}</div>
                    <div className="text-lg font-black text-white">₹{price}</div>
                    <div className={`text-[10px] mt-2 font-bold uppercase ${
