  ...
  return (
    <div className="absolute inset-0 z-0 pointer-events-none opacity-40 overflow-hidden flex items-center justify-center mix-blend-screen">
      <svg viewBox="0 0 1000 500" className="w-full max-w-[1400px] h-auto stroke-slate-800">
        ...
        {nodes.map((node, i) => (
          <g key={`node-${i}`}>
            <circle cx={node.cx} cy={node.cy} r="4" className="fill-[#080c14] stroke-amber-500 stroke-2" />
            <motion.circle 
              cx={node.cx} 
              cy={node.cy} 
              r="12" 
            ...
            />
            <text x={node.cx} y={node.cy - 12} className="fill-slate-400 text-[10px] font-mono tracking-widest uppercase text-anchor-middle">
              {node.label}
            </text>
          </g>
        ))}
