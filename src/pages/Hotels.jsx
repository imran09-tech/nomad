...
import { Building2, Calendar, Users, MapPin, Search, ArrowRight, Star, Heart, Compass, CheckCircle } from 'lucide-react';
  ...
  const mockHotels = [
    {
      ...
      description: 'A monument to modern Japanese architecture, blending traditional minimalism with high-end luxury overlooking the Imperial Palace gardens.',
    ...
    },
    {
      ...
      description: 'Perched on rugged volcanic cliffs 150 meters above the Indian Ocean, offering Italian design blended with authentic Balinese heritage.',
    ...
    },
    {
      ...
      description: 'An elite village of palatial riads crafted by Moroccan artisans, featuring private plunge pools and absolute butler service.',
    ...
    },
    {
      ...
      description: 'Overwater retreats featuring retractable roofs to stargaze, private waterslides into the lagoon, and complete barefoot luxury.',
    ...
    >
      ...
      <div className="max-w-6xl mx-auto space-y-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2">
          <div>
            <div className="text-[10px] text-slate-500 font-mono tracking-widest uppercase mb-2">Hospitality Network</div>
            <h1 className="text-5xl md:text-6xl font-black text-white tracking-tighter leading-none">
              Luxury <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-300 to-emerald-500">Estates</span>
            </h1>
            <p className="text-slate-400 font-medium tracking-wide mt-2">Book curated sanctuaries of unparalleled prestige.</p>
          </div>
          <NeonBadge variant="emerald" className="w-fit">Elite Portfolio Online</NeonBadge>
        </div>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="flex flex-col space-y-2">
              <label className="text-[9px] text-slate-400 font-mono font-bold tracking-widest uppercase">Sanctuary Location</label>
            ...
            {/* Check-In */}
            <div className="flex flex-col space-y-2">
              <label className="text-[9px] text-slate-400 font-mono font-bold tracking-widest uppercase">Arrival</label>
              <div className="relative flex items-center">
            ...
            {/* Check-Out */}
            <div className="flex flex-col space-y-2">
              <label className="text-[9px] text-slate-400 font-mono font-bold tracking-widest uppercase">Departure</label>
              <div className="relative flex items-center">
                <Calendar className="w-4 h-4 text-emerald-400 absolute left-4" />
                <input 
                  type="date" 
            ...
            {/* Guests */}
            <div className="flex flex-col space-y-2">
              <label className="text-[9px] text-slate-400 font-mono font-bold tracking-widest uppercase">Guests</label>
              <div className="relative flex items-center">
              ...
              <TicketStubSummary 
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                      <div className="text-[10px] text-slate-500 font-mono tracking-widest uppercase mb-1 flex items-center gap-1">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400" /> CONFIRMED PRIVATE RESERVATION
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-2 border-t md:border-t-0 md:border-l border-slate-800/60 pt-4 md:pt-0 md:pl-8 text-left">
                      <div>
                        <span className="block text-[9px] text-slate-500 font-mono tracking-widest uppercase">Suite Selected</span>
                        <span className="text-sm font-extrabold text-emerald-400">{selectedRoom.name}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] text-slate-500 font-mono tracking-widest uppercase">Check-In</span>
                        <span className="text-sm font-extrabold text-white">{searchState.checkin}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] text-slate-500 font-mono tracking-widest uppercase">Guests</span>
                        <span className="text-sm font-extrabold text-white">{searchState.guests}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] text-slate-500 font-mono tracking-widest uppercase">Status</span>
                        <span className="text-sm font-extrabold text-emerald-400">Exclusive Reserved</span>
                      </div>
                    </div>
                  </div>
                }
                rightContent={
                  <div className="text-center">
                    <div className="text-[10px] text-slate-500 font-mono tracking-widest uppercase mb-1">Total Stay Rate</div>
                    <div className="text-4xl font-black text-white tracking-tighter mb-5">${selectedRoom.price}</div>
                    <ShimmerButton 
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-extrabold text-white tracking-wide">Elite Estates Portfolio</h3>
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute inset-0 border rounded-3xl pointer-events-none transition-colors duration-500 ${
                    isSelected ? 'border-emerald-500/50 bg-[#090d16]/40' : 'border-slate-800/20 hover:border-slate-700/40'
                  }`} />
                  ...
                  <div className="absolute top-6 left-6 right-6 flex justify-between items-start z-10">
                    ...
                    <button 
                      ...
                      className={`p-2 rounded-full backdrop-blur-md border border-slate-700/30 transition-all ${
                        isWished ? 'bg-rose-500/20 border-rose-500 text-rose-500' : 'bg-slate-900/40 text-slate-400 hover:text-white'
                      }`}
                    >
                  ...
                  <div className="relative z-10 mt-auto">
                    <div className="flex justify-between items-end mb-3">
                      <div>
                        <div className="text-[10px] text-slate-400 font-mono font-bold tracking-widest uppercase">{hotel.location}</div>
                        <h3 className="text-3xl font-black text-white tracking-tighter mt-1">{hotel.name}</h3>
                      </div>
                      <div className="text-right">
                        <span className="block text-[9px] text-slate-500 font-mono tracking-widest uppercase">Per Night</span>
                    ...
                    <AnimatePresence>
                      {isSelected && (
                        ...
                        >
                          <div className="text-[10px] text-slate-500 font-mono tracking-widest uppercase mb-3">Select Premium Suite</div>
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            {Object.entries(hotel.rooms).map(([roomName, roomPrice]) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="text-[10px] font-bold text-slate-400 leading-tight">{roomName}</div>
                                  <div className="text-base font-black text-white mt-1">${roomPrice}</div>
                                </motion.div>
                              );
                            })}
                          </div>
